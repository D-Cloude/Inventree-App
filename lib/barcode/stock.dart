...
/*
 * Barcode handler for scanning a StockItem during a stocktake (재물조사).
 *
 * - Validates that the scanned barcode matches a valid StockItem
 * - Calls the provided callback with the loaded StockItem
 * - Scanner stays open after each scan to allow multiple items
 */
class StocktakeScanItemHandler extends BarcodeHandler {
  ...
  @override
  String getOverlayText(BuildContext context) => L10().stocktakeScanItem;
  ...
  Future<void> onBarcodeMatched(Map<String, dynamic> data) async {
    if (data.containsKey("stockitem")) {
      ...
      if (pk > 0) {
        final item =
            await InvenTreeStockItem().get(pk) as InvenTreeStockItem?;
